...
void CLI::selectMode() {
    ...
    std::cout << "\nCommands: 'undo', 'eval', 'savepgn <file>', 'perft <depth>', 'quit'\n\n";
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); // flush newline for getline
}
...
void CLI::handleCommand(const std::string& cmd) {
    ...
    if (word=="undo") {
        ...
        std::cout << "Current evaluation: " << (score>=0?"+":"") << score << " centipawns (";
        if (score>50) std::cout<<"White better";
        else if (score<-50) std::cout<<"Black better";
        else std::cout<<"roughly equal";
        std::cout<<")\n";
        std::cout << "Game phase: " << Eval::gamePhase(board) << "/24\n";
